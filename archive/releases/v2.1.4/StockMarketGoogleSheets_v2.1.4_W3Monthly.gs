...
function refreshHistoricalDate_(dateText) {
  ...
  try {
    ...
    writeHistoryLog_(
      ...
      `週五=${optionOi.weeklyFriday.contract}`
    );
  } catch (error) {
...
function fetchTwseTaiexForDate_(dateText) {
  ...
  const response = fetchUrl_(CONFIG.TWSE_URL + monthKey);
  const json = JSON.parse(response.getContentText('UTF-8'));

  if (!Array.isArray(json.data) || !Array.isArray(json.fields)) {
    throw new Error('證交所未回傳歷史加權指數資料。');
  }

  const dateIndex = findHeaderIndex_(json.fields, ['日期']);
  const openIndex = findHeaderIndex_(json.fields, ['開盤指數', '開盤']);
  const highIndex = findHeaderIndex_(json.fields, ['最高指數', '最高']);
  const lowIndex = findHeaderIndex_(json.fields, ['最低指數', '最低']);
  const closeIndex = findHeaderIndex_(json.fields, ['收盤指數', '收盤']);
