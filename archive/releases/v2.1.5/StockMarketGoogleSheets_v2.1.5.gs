 ...
 * 4. 首次執行需授權
 *
 * v2.1.5：大盤本月成功後不再請求上月；共用歷史解析與官方備援。
 *         9 月結算當日起價平改採現貨；履約價必須實際掛牌且具買賣權。
 *
 * v2.1.4：修正週三到期選擇權的契約序列。
 *         W 區塊改為「最近一個尚未到期的週三到期序列」：
 *         W1 → W2 → 當月月選（第三週）→ W4 → W5。
 *         因期交所沒有 W3，第三個週三由當月月選契約承接。
 *         到期日當天仍視為已到期，直接切換至下一個週三序列。
 *
 * v2.1.3：新增「歷史區間重新抓取／修復」功能。
 *         可指定起訖日期，逐交易日重新取得現貨、台指期、散戶多空比、
 *         P/C 比、大額法人未平倉、選擇權 OI／IV，成功取得且日期完全
 *         相符時才覆寫原始資料，並同步更新該日總表。採小批次自動續跑，
 *         避免 Apps Script 執行逾時；每個來源結果寫入「歷史修復紀錄」。
 *
 * v2.1.2：調整 TXO 契約選擇策略。若資料日期已達契約到期日，
 *         當日即不再採用該到期契約，而改取下一個尚未到期的有效契約。
 *         此規則同步套用月選、W 週三選、F 週五選的 OI 與 IV。
 *         W 契約仍僅允許 W1、W2、W4、W5，不存在 W3。
 *
...
const CONFIG = {
  ...
  SUMMARY_SHEET: '總表',
  LOG_SHEET: '執行紀錄',
  HISTORY_LOG_SHEET: '歷史修復紀錄',
  TRIGGER_HOUR: 20,
  TRIGGER_MINUTE: 0,

  // 歷史修復採小批次執行，避免 Apps Script 單次執行時間過長。
  HISTORY_BATCH_TRADING_DAYS: 2,
  HISTORY_MAX_CALENDAR_DAYS: 93,
  HISTORY_CONTINUE_DELAY_MS: 60 * 1000,
...
function onOpen() {
  SpreadsheetApp.getUi()
    ...
    .addSeparator()
    .addItem('重新抓取歷史區間', 'startHistoricalRefresh')
    .addItem('查看歷史修復狀態', 'showHistoricalRefreshStatus')
    .addItem('取消歷史修復', 'cancelHistoricalRefresh')
    .addToUi();
}
...
function initializeSheets() {
  ...
  const historyLogSheet = getOrCreateSheet_(ss, CONFIG.HISTORY_LOG_SHEET);
  initializeHeader_(historyLogSheet, HISTORY_LOG_HEADERS);
  historyLogSheet.setFrozenRows(1);
  historyLogSheet.getRange('A:A').setNumberFormat('yyyy/mm/dd hh:mm:ss');
  historyLogSheet.getRange('B:B').setNumberFormat('@');
  historyLogSheet.autoResizeColumns(1, HISTORY_LOG_HEADERS.length);
}
...
function updateMarketData() {
  ...
  try {
    ...
    try {
      ...
      optionOi.status += '／' + describeOptionAtm_(optionOi);
      ...
      if (!optionOi.atmComplete) errors.push(`選擇權ATM/IV：${describeOptionAtm_(optionOi)}`);
    } catch (error) {
...
function fetchTwseTaiex_() {
  ...
  const currentMonth = today.replace(/-/g, '').slice(0, 6) + '01';
  let records = fetchTwseIndexRows_(currentMonth).filter((row) => row.date <= today);

  // 本月有有效資料就立即使用；不再讓非必要的上月請求使本月失敗。
  // 本月請求失敗會拋錯；只有本月確實尚無資料時才查上月，避免掩蓋故障。
  if (!records.length) {
    const previous = parseIsoDateAtNoon_(today);
    previous.setDate(1);
    previous.setMonth(previous.getMonth() - 1);
    records = fetchTwseIndexRows_(
      Utilities.formatDate(previous, CONFIG.TIMEZONE, 'yyyyMM') + '01'
    );
  }
  if (!records.length) throw new Error('證交所未回傳可用的加權指數資料。');
  records.sort((a, b) => b.date.localeCompare(a.date));
  const latest = records[0];
  return {
    ...latest,
    volume: fetchTwseTradeVolume_(latest.date),
    product: '大盤現貨指數',
    contractMonth: '',
    source: '臺灣證券交易所',
  };
...
/** 最新行情與歷史修復共用；有限次重試並使用官方另一報表路徑備援。 */
function fetchTwseIndexRows_(monthKey) {
  const urls = [
    CONFIG.TWSE_URL + monthKey,
    'https://www.twse.com.tw/indicesReport/MI_5MINS_HIST?response=json&date=' + monthKey,
  ];
  const errors = [];
  for (const url of urls) {
    for (let attempt = 0; attempt < 2; attempt += 1) {
      try {
        const json = JSON.parse(fetchUrl_(url).getContentText('UTF-8'));
        if (json.stat && /沒有符合條件|查無資料/.test(json.stat)) return [];
        const containers = [json].concat(Array.isArray(json.tables) ? json.tables : []);
        const records = [];
        let foundTable = false;
        containers.forEach((table) => {
          if (!Array.isArray(table.fields) || !Array.isArray(table.data)) return;
          const indices = [
            ['日期'], ['開盤指數', '開盤'], ['最高指數', '最高'],
            ['最低指數', '最低'], ['收盤指數', '收盤'],
          ].map((aliases) => findHeaderIndex_(table.fields, aliases, false));
          if (indices.some((index) => index < 0)) return;
          foundTable = true;
          table.data.forEach((row) => {
            const date = parseTwseDate_(row[indices[0]]);
            const [open, high, low, close] = indices.slice(1).map((index) => parseNumber_(row[index]));
            if (date && date.replace(/-/g, '').slice(0, 6) === monthKey.slice(0, 6) &&
          ...
          if (table.data.length && !records.length) throw new Error('日期或 OHLC 驗證失敗');
        });
        if (!foundTable) throw new Error('缺少加權指數資料表');
        return records;
      } catch (error) {
        errors.push(`${url} 第${attempt + 1}次：${error.message}`);
        if (attempt === 0) Utilities.sleep(500);
      }
    }
  }
  throw new Error(`加權指數擷取失敗：${errors.join('；')}`);
}
...
/**
 * 從所有標準化 TXO 資料列，找出最近交易日的月選、週三序列與 F 前端有效契約。
 *
 * 週三序列規則（v2.1.4）：
 * W1 → W2 → 當月月選（第三週）→ W4 → W5
 *
 * 期交所沒有 W3；第三個週三由當月月選契約承接。因此「週選擇權 W」欄位
 * 在第三週期間會顯示 YYYYMM 月選契約代碼，這是刻意的正確行為。
 *
 * v2.1.2 起先排除到期日 <= 資料日期的契約；因此到期日當天直接改用下一契約。
 * 在尚未到期的契約中，優先選擇至少有正數 OI 的最近到期契約；若全部為零，
 * 才退回該類別最近到期的已掛牌契約。
 */
function buildTaifexOptionOiSummary_(rows) {
  ...
  // 「W」代表最近一個尚未到期的週三到期序列。
  // 第三個週三沒有 W3，改由當月月選 YYYYMM 承接。
  // 因此候選契約需同時包含月選與 W1/W2/W4/W5，再依實際到期日排序。
  ...
  try {
    ...
    [monthly, weeklyWednesday, weeklyFriday].forEach((summary) => {
      try {
        attachOptionAtmMetrics_(summary, latestRows, atmContext, latestDate);
      } catch (error) {
        atmWarning = mergeUniqueText_(atmWarning, `${summary.contract}：${error.message}`, '；');
      }
    });
  } catch (error) {
...
function selectOptionContractSummary_(rows, pattern, label, tradeDate) {
  const normalizedTradeDate = parseGenericDate_(tradeDate) || String(tradeDate || '');
  ...
  // 若來源未帶契約到期日，依 TXO 掛牌代碼推回名目到期日，並依期交所
  // 「最後交易日遇休市順延至次一營業日」規則調整。這是備援邏輯；
  // 正常情況仍優先使用期交所行情資料內的契約到期日。
  allGroups.forEach((group) => {
    if (!group.expiryDate) {
      group.expiryDate = inferTxoExpiryDate_(group.contract);
    }
  });

  // 核心規則：到期日當天即視為該契約已結束，不再拿來做 OI / IV 日報。
  // 因此只接受 expiryDate > tradeDate，而不是 >=。
  const activeGroups = allGroups.filter(
    (group) =>
      group.expiryDate &&
      normalizedTradeDate &&
      group.expiryDate > normalizedTradeDate
  );

  if (activeGroups.length === 0) {
    throw new Error(
      `${label} 在 ${normalizedTradeDate || tradeDate} 找不到下一個尚未到期的契約。`
    );
  }

  const groupsWithOi = activeGroups.filter(
    (group) => group.positiveOiTotal > 0
  );
  const pool = groupsWithOi.length > 0 ? groupsWithOi : activeGroups;
 ...
 * 依 TXO 契約代碼推算名目到期日：
 * - 月選 YYYYMM：第 3 個星期三
 * - W1/W2/W4/W5：該月第 1/2/4/5 個星期三（沒有 W3）
 * - F1~F5：該月第 1~5 個星期五
 * 若名目最後交易日為休市日，順延到最近次一交易日。
 */
function inferTxoExpiryDate_(contractMonth) {
  const text = String(contractMonth || '').trim().toUpperCase();
  const match = text.match(/^(\d{4})(\d{2})(?:W([1245])|F([1-5]))?$/);

  if (!match) return '';

  const year = Number(match[1]);
  const month = Number(match[2]);
  const wNumber = match[3] ? Number(match[3]) : null;
  const fNumber = match[4] ? Number(match[4]) : null;
  const targetWeekday = fNumber ? 5 : 3; // 星期五或星期三
  const nth = fNumber || wNumber || 3; // 月選為第三個星期三
  ...
  const nominal = new Date(year, month - 1, dayOfMonth, 12, 0, 0);

  // W5 / F5 只有在該月份真的有第 5 個相應星期時才有效。
  if (nominal.getMonth() !== month - 1) return '';

  const adjusted = new Date(nominal.getTime());
  let guard = 0;
  while (!isTwseTradingDate_(adjusted) && guard < 10) {
  ...
  return Utilities.formatDate(adjusted, CONFIG.TIMEZONE, 'yyyy-MM-dd');
}


/**
 * 5 月起至 9 月月選結算前採期貨；9 月結算當日起及其他月份採現貨。
 */
function getOptionAtmContext_(date) {
  ...
  const month = Number(match[2]);
  const septemberExpiry = month === 9 ? inferTxoExpiryDate_(match[1] + '09') : '';
  if (month === 9 && !septemberExpiry) throw new Error('無法判定9月結算日，停止價平計算。');
  const useFuture = month >= 5 && month <= 9 &&
    (month !== 9 || date < septemberExpiry);
...
function attachOptionAtmMetrics_(summary, rows, atmContext, tradeDate) {
  const contractRows = rows.filter(
    (row) => row.contractMonth === summary.contract && row.date === tradeDate &&
      row.contract === 'TXO' && isRegularSession_(row.session)
  );
  summary.atmStrike = selectListedAtmStrike_(contractRows, atmContext);
...
/** 保留 50 點目標；未掛牌時依原始基準價找最近的雙邊檔位，等距取高。 */
function selectListedAtmStrike_(rows, context) {
  const calls = new Set(rows.filter((row) => row.side === 'Call').map((row) => row.strike));
  const strikes = Array.from(new Set(rows
    .filter((row) => row.side === 'Put' && calls.has(row.strike))
    .map((row) => row.strike)))
    .filter((strike) => Number.isFinite(strike) && strike > 0);
  if (!strikes.length) throw new Error('找不到 Call／Put 皆有掛牌的履約價');
  if (strikes.includes(context.atmStrike)) return context.atmStrike;
  const reference = context.atmReferencePrice;
  if (!Number.isFinite(reference) || reference <= 0) throw new Error('價平基準價無效');
  if (reference < Math.min(...strikes) || reference > Math.max(...strikes)) {
    throw new Error('價平基準超出雙邊掛牌範圍，請檢查資料完整性');
  }
  return strikes.sort((a, b) => Math.abs(a - reference) - Math.abs(b - reference) || b - a)[0];
}

function describeOptionAtm_(record) {
  const details = [record.weeklyWednesday, record.weeklyFriday, record.monthly]
    .map((item) => `${item.contract} 基準=${item.atmUnderlyingSource || '缺少'} ${valueOrBlank_(item.atmUnderlying)} 履約價=${valueOrBlank_(item.atmStrike)}`);
  if (record.atmWarning) details.push(record.atmWarning);
  return details.join('；');
}
...
const HISTORY_STATE_KEY = 'MARKET_HISTORY_REPAIR_STATE';
const HISTORY_TRIGGER_HANDLER = 'continueHistoricalRefresh';

/**
 * 從選單啟動歷史修復。
 * 僅允許「今天以前」的日期，避免把尚未完整結算的今日資料當作歷史值。
 */
function startHistoricalRefresh() {
  ...
  const startResponse = ui.prompt(
    '歷史資料修復',
    '請輸入開始日期（YYYY-MM-DD），例如 2026-08-01：',
  ...
  const endResponse = ui.prompt(
    '歷史資料修復',
    '請輸入結束日期（YYYY-MM-DD）。結束日期需早於今天：',
    ui.ButtonSet.OK_CANCEL
  );
  if (endResponse.getSelectedButton() !== ui.Button.OK) return;

  const startDate = normalizeHistoryInputDate_(
    startResponse.getResponseText()
  );
  const endDate = normalizeHistoryInputDate_(
  ...
  if (endDate >= today) {
    ui.alert(
      `歷史修復只接受今天以前的日期。今天為 ${today}。`
    );
    return;
  }

  const calendarDays = daysBetweenIsoDates_(startDate, endDate) + 1;
  if (calendarDays > Number(CONFIG.HISTORY_MAX_CALENDAR_DAYS || 93)) {
    ui.alert(
      `單次歷史修復最多 ${CONFIG.HISTORY_MAX_CALENDAR_DAYS} 個日曆日。` +
      '較長期間請分段執行。'
    );
    return;
  }
  ...
  initializeSheets();
  deleteTriggersByHandler_(HISTORY_TRIGGER_HANDLER);
  ...
  saveHistoryRefreshState_(state);

  writeHistoryLog_(
    '',
    '系統',
    '開始',
    `開始重新抓取 ${startDate} ～ ${endDate}`
  );
  ...
  ui.alert(
    ...
    [
      ...
      `每批處理 ${CONFIG.HISTORY_BATCH_TRADING_DAYS} 個交易日。`,
      '完成一批後會自動建立一次性觸發器續跑。',
      '可從「台股行情工具 → 查看歷史修復狀態」查看進度。',
      '',
      '只有成功取得且資料日期完全相符的來源才會覆寫；抓取失敗不會清空舊資料。',
    ].join('\n'),
    ui.ButtonSet.OK
  );

  continueHistoricalRefresh();
}

/**
 * 批次處理歷史資料。可由選單起始函式或一次性 trigger 呼叫。
 */
function continueHistoricalRefresh() {
  const lock = LockService.getScriptLock();

  if (!lock.tryLock(30000)) {
    scheduleHistoricalRefreshContinuation_();
    return;
  }
  ...
  try {
    ...
    const state = loadHistoryRefreshState_();
    if (!state || state.status !== '執行中') {
      return;
    }

    let cursor = parseIsoDateAtNoon_(state.cursorDate);
    const end = parseIsoDateAtNoon_(state.endDate);
    let batchTradingDays = 0;

    while (
      cursor &&
      end &&
      cursor.getTime() <= end.getTime() &&
      batchTradingDays < Number(CONFIG.HISTORY_BATCH_TRADING_DAYS || 2)
    ) {
      ...
      if (isTwseTradingDate_(cursor)) {
        const dayResult = refreshHistoricalDate_(dateText);
        state.processedTradingDays += 1;

        if (dayResult.successCount > 0 && dayResult.errorCount === 0) {
          state.successfulDates += 1;
        } else if (dayResult.successCount > 0) {
          state.partialDates += 1;
        } else {
          state.failedDates += 1;
        }
    ...
    const nextCursor = cursor
      ? Utilities.formatDate(cursor, CONFIG.TIMEZONE, 'yyyy-MM-dd')
      : state.endDate;

    state.cursorDate = nextCursor;

    if (!cursor || cursor.getTime() > end.getTime()) {
      state.status = '完成';
      ...
      writeHistoryLog_(
        ...
        '完成',
        [
          `區間 ${state.startDate} ～ ${state.endDate}`,
          `交易日 ${state.processedTradingDays}`,
          `完整成功 ${state.successfulDates}`,
          `部分成功 ${state.partialDates}`,
          `完全失敗 ${state.failedDates}`,
        ].join('；')
      );
      writeLog_(
        '成功',
        `歷史修復完成：${state.startDate}～${state.endDate}；` +
        `完整 ${state.successfulDates}、部分 ${state.partialDates}、失敗 ${state.failedDates}。`
      );
    } else {
      saveHistoryRefreshState_(state);
      scheduleHistoricalRefreshContinuation_();
    ...
    writeHistoryLog_('', '系統', '中斷', error.stack || error.message);
    writeLog_('失敗', `歷史修復中斷：${error.message}`);
    throw error;
  } finally {
    lock.releaseLock();
  }
...
/**
 * 重新抓取單一交易日。每個來源獨立處理；某來源失敗不影響其他來源。
 */
function refreshHistoricalDate_(dateText) {
  const results = [];
  const errors = [];
  const summaryDates = new Set();
  ...
  // 1. 大盤現貨 + 成交量
  try {
    const spot = fetchTwseTaiexForDate_(dateText);
    spot.status = '歷史重新驗證';
    upsertMarketRow_(spot);
    summaryDates.add(dateText);
    results.push('大盤現貨');
    writeHistoryLog_(dateText, '大盤現貨', '成功覆寫', spot.source);
  } catch (error) {
    errors.push(`大盤現貨：${error.message}`);
    writeHistoryLog_(dateText, '大盤現貨', '保留舊值', error.message);
  }

  // 2. 台指期日盤
  try {
    const future = fetchTaifexTxForDate_(dateText);
    future.status = '歷史重新驗證';
    upsertMarketRow_(future);
    summaryDates.add(dateText);
    results.push('台指期');
    writeHistoryLog_(
      ...
      '台指期',
      '成功覆寫',
      `${future.contractMonth}；${future.source}`
    );
  } catch (error) {
    errors.push(`台指期：${error.message}`);
    writeHistoryLog_(dateText, '台指期', '保留舊值', error.message);
  }

  // 3. 小台／微台散戶多空比
  try {
    const ratios = fetchTaifexRetailRatiosForDate_(dateText);

    upsertChipFields_({
      date: dateText,
      miniRetailRatio: ratios.mini.ratio,
      source: '臺灣期貨交易所－歷史官方未平倉資料推算（小台指）',
      status: '歷史重新驗證',
    });
    upsertChipFields_({
      date: dateText,
      microRetailRatio: ratios.micro.ratio,
      source: '臺灣期貨交易所－歷史官方未平倉資料推算（微台指）',
      status: '歷史重新驗證',
    });
    ...
    writeHistoryLog_(
      ...
      '小台／微台散戶多空比',
      '成功覆寫',
      `MTX ${ratios.mini.ratio}%；TMF ${ratios.micro.ratio}%`
    );
  } catch (error) {
    errors.push(`散戶多空比：${error.message}`);
    writeHistoryLog_(
  ...
  // 4. Put/Call OI ratio
  try {
    const pc = fetchTaifexPcRatioForDate_(dateText);
    upsertChipFields_({
      date: dateText,
      pcOpenInterestRatio: pc.openInterestRatio,
      source: '臺灣期貨交易所－臺指選擇權 Put/Call 比（歷史重新驗證）',
      status: '歷史重新驗證',
    });
    summaryDates.add(dateText);
    results.push('P/C未平倉比');
    writeHistoryLog_(
      dateText,
      'P/C未平倉比',
      '成功覆寫',
      `${pc.openInterestRatio}%`
    );
  } catch (error) {
    errors.push(`P/C未平倉比：${error.message}`);
    writeHistoryLog_(dateText, 'P/C未平倉比', '保留舊值', error.message);
  }

  // 5. 聚財網大額／法人未平倉
  try {
    const positions = fetchWearnTaifexPositionsForDate_(dateText);
    upsertChipFields_({
      date: dateText,
      top5: positions.top5,
      top10: positions.top10,
      ...
      investmentTrust: positions.investmentTrust,
      dealer: positions.dealer,
      source: '聚財網－台指期未平倉（歷史重新驗證）',
      status: '歷史重新驗證',
    });
    summaryDates.add(dateText);
    results.push('大額法人未平倉');
    writeHistoryLog_(
      dateText,
      '大額法人未平倉',
      '成功覆寫',
  ...
  // 6. TXO OI / 價平 / IV
  // ATM 與 IV 依賴當日現貨／期貨，因此放在行情修復之後。
  try {
    const optionOi = fetchTaifexOptionOiMaximaForDate_(dateText);
    optionOi.status = optionOi.atmComplete
      ? '歷史重新驗證'
      : '歷史重新驗證／價平成交價或IV部分缺漏';
    optionOi.status += '／' + describeOptionAtm_(optionOi);
    upsertOptionOiRow_(optionOi);
    summaryDates.add(dateText);
    results.push('選擇權OI/IV');
    writeHistoryLog_(
      dateText,
      '選擇權 OI／IV',
      optionOi.atmComplete ? '成功覆寫' : '部分覆寫／ATM或IV缺漏',
      `月=${optionOi.monthly.contract}／` +
      `週三=${optionOi.weeklyWednesday.contract}／` +
      `週五=${optionOi.weeklyFriday.contract}；${describeOptionAtm_(optionOi)}`
    );
    if (!optionOi.atmComplete) errors.push(`選擇權ATM/IV：${describeOptionAtm_(optionOi)}`);
  } catch (error) {
    errors.push(`選擇權OI/IV：${error.message}`);
    writeHistoryLog_(
      dateText,
      '選擇權 OI／IV',
      '保留舊值',
  ...
  if (summaryDates.size > 0) {
    try {
      upsertSummaryDates_([dateText]);
      writeHistoryLog_(dateText, '總表', '成功更新', '依修復後來源表重新計算。');
    } catch (error) {
      errors.push(`總表：${error.message}`);
      writeHistoryLog_(dateText, '總表', '失敗', error.message);
    }
  }
 ...
 * 大盤指定日期 OHLC + 成交量。
 */
function fetchTwseTaiexForDate_(dateText) {
  const date = parseIsoDateAtNoon_(dateText);
  if (!date) throw new Error(`日期格式錯誤：${dateText}`);

  const monthKey =
    Utilities.formatDate(date, CONFIG.TIMEZONE, 'yyyyMM') + '01';
  const matched = fetchTwseIndexRows_(monthKey).find((row) => row.date === dateText);

  if (!matched) {
    throw new Error(`證交所找不到 ${dateText} 的加權指數 OHLC。`);
  }

  return {
    ...matched,
    volume: fetchTwseTradeVolume_(dateText),
    product: '大盤現貨指數',
    contractMonth: '',
    source: '臺灣證券交易所－歷史資料',
  };
 ...
 * 台指期指定日期日盤 OHLC。
 */
function fetchTaifexTxForDate_(dateText) {
  const html = fetchTaifexFuturesHistoricalHtml_('TX', dateText);
  const parsed = parseTaifexFutureHistoricalPage_(html, 'TX', dateText);

  const rows = parsed.rows.filter(
    (row) =>
      /^\d{6}$/.test(row.contractMonth) &&
      [row.open, row.high, row.low, row.close].every(Number.isFinite)
  );

  if (!rows.length) {
    throw new Error(`${dateText} 找不到 TX 單式日盤行情。`);
  }
 ...
 * 指定日期 MTX / TMF 散戶多空比。
 */
function fetchTaifexRetailRatiosForDate_(dateText) {
  const institutions = fetchTaifexInstitutionalOpenInterestForDate_(dateText);
  const miniMarket = fetchTaifexTotalOpenInterestForDate_('MTX', dateText);
  const microMarket = fetchTaifexTotalOpenInterestForDate_('TMF', dateText);

  return {
    mini: calculateRetailRatio_(
      miniMarket,
      institutions.MTX,
      '小台指'
    ),
    micro: calculateRetailRatio_(
      microMarket,
      institutions.TMF,
      '微台指'
    ),
 ...
 * 指定日期全市場未平倉量。
 */
function fetchTaifexTotalOpenInterestForDate_(contractCode, dateText) {
  const html = fetchTaifexFuturesHistoricalHtml_(contractCode, dateText);
  const parsed = parseTaifexFutureHistoricalPage_(
    html,
    contractCode,
    dateText
  );
  ...
  parsed.rows.forEach((row) => {
    if (
      row.contractMonth &&
      !String(row.contractMonth).includes('/') &&
      Number.isFinite(row.openInterest)
    ) {
      total += row.openInterest;
      count += 1;
    }
  });

  if (!count || total <= 0) {
    throw new Error(`${contractCode} ${dateText} 找不到正數全市場 OI。`);
  }
...
/**
 * 指定日期三大法人 MTX / TMF 未平倉。
 */
function fetchTaifexInstitutionalOpenInterestForDate_(dateText) {
  const slashDate = isoToSlashDate_(dateText);
  const attempts = [
    {
      url: CONFIG.TAIFEX_INSTITUTIONAL_URL,
      method: 'post',
      payload: {
        queryType: '1',
        doQuery: '1',
    ...
    },
    {
      url:
        CONFIG.TAIFEX_INSTITUTIONAL_URL +
        '?queryType=1&doQuery=1&queryDate=' +
        encodeURIComponent(slashDate),
      method: 'get',
    },
  ];
  ...
  for (const attempt of attempts) {
    try {
      const html = fetchHistoryHtmlAttempt_(attempt, ['UTF-8']);
      const parsed = parseTaifexInstitutionalHtmlForDate_(html, dateText);
      return parsed;
    } catch (error) {
      lastError = error;
    }
  ...
  throw lastError || new Error(`${dateText} 三大法人歷史資料取得失敗。`);
}

/**
 * 指定日期 P/C 未平倉量比。
 * 先讀目前表格；若目標日期不在頁面可見範圍，再以起訖日查詢。
 */
function fetchTaifexPcRatioForDate_(dateText) {
  const parseExact = (html) => {
    const plainText = normalizeWhitespace_(
      decodeHtmlEntities_(stripHtml_(html))
    );
    if (!plainText.includes('買賣權未平倉量比率')) {
      throw new Error('頁面找不到 P/C 未平倉量比欄位。');
    }
    ...
    const matched = rows
      ...
      .find(
        (row) =>
          row.date === dateText &&
          Number.isFinite(row.openInterestRatio)
      );

    if (!matched) {
      throw new Error(`P/C 表格未包含 ${dateText}。`);
    }
  ...
  try {
    const currentHtml = fetchHtmlWithCacheBust_(
      CONFIG.TAIFEX_PC_RATIO_URL,
      ['UTF-8']
    );
    return parseExact(currentHtml);
  } catch (firstError) {
    const slashDate = isoToSlashDate_(dateText);
    const attempts = [
      {
        url: CONFIG.TAIFEX_PC_RATIO_URL,
        method: 'post',
        payload: {
          queryStartDate: slashDate,
          queryEndDate: slashDate,
        },
      },
      {
        url:
    ...
    for (const attempt of attempts) {
      try {
        return parseExact(fetchHistoryHtmlAttempt_(attempt, ['UTF-8']));
      } catch (error) {
        lastError = error;
      }
    }
...
/**
 * 聚財網目前的資料頁本身會列出多個歷史交易日，因此直接找指定日期。
 * 若日期已超出該頁保留範圍，安全地保留原值。
 */
function fetchWearnTaifexPositionsForDate_(dateText) {
  const html = fetchHtmlWithCacheBust_(
    CONFIG.WEARN_TAIFEX_URL,
    ['UTF-8', 'Big5'],
    ['前五大', '前十大', '外資', '投信', '自營商']
...
/**
 * TXO 指定日期完整日盤行情，套用 v2.1.2 契約挑選規則與 v2.1 IV 計算。
 */
function fetchTaifexOptionOiMaximaForDate_(dateText) {
  const html = fetchTaifexOptionsHistoricalHtml_(dateText);
  const plainText = normalizeWhitespace_(
    decodeHtmlEntities_(stripHtml_(html))
  );
  ...
  if (
    ...
    throw new Error(`${dateText} 選擇權頁面不是一般交易時段。`);
  }
  ...
  if (headerRowIndex < 0) {
    throw new Error(`${dateText} 找不到選擇權完整行情標題列。`);
  }
  ...
  tableRows.slice(headerRowIndex + 1).forEach((cells) => {
    if (!Array.isArray(cells) || cells.length <= maxIndex) return;
    ...
    if (contract !== 'TXO' || !isTrackedTxoContract_(contractMonth)) return;
    ...
    if (!Number.isFinite(strike) || !side) return;
  ...
  if (!normalizedRows.length) {
    throw new Error(`${dateText} 找不到 TXO 完整日盤資料列。`);
  }

  const result = buildTaifexOptionOiSummary_(normalizedRows);

  if (result.date !== dateText) {
    throw new Error(
      `選擇權頁面日期不符：要求 ${dateText}，取得 ${result.date}`
    );
  }

  result.source =
    '臺灣期貨交易所官網－選擇權歷史完整日盤行情；' +
    'IV依Black-Scholes反推（交易日年化、最後成交價）';
  return result;
}
...
/**
 * 取得指定日期期貨日盤歷史 HTML。
 * 同時嘗試 Excel 外部資料頁 GET 與查詢頁 POST；每次都由後續 parser
 * 驗證頁面實際資料日期，避免網站忽略 queryDate 時誤覆寫。
 */
function fetchTaifexFuturesHistoricalHtml_(contractCode, dateText) {
  const slashDate = isoToSlashDate_(dateText);
  const excelUrl =
  ...
  const attempts = [
    {
      url:
        excelUrl +
        '?queryType=2&marketCode=0&commodity_id=' +
        encodeURIComponent(contractCode) +
        '&queryDate=' +
        encodeURIComponent(slashDate),
      method: 'get',
    },
    {
      url:
        reportUrl +
        '?queryType=2&marketCode=0&commodityId=' +
        encodeURIComponent(contractCode) +
        '&queryDate=' +
        encodeURIComponent(slashDate),
      method: 'get',
  ...
  for (const attempt of attempts) {
    try {
      const html = fetchHistoryHtmlAttempt_(attempt, ['UTF-8']);
      parseTaifexFutureHistoricalPage_(html, contractCode, dateText);
      return html;
    } catch (error) {
      lastError = error;
    }
  ...
  throw lastError || new Error(`${contractCode} ${dateText} 歷史行情取得失敗。`);
}

/**
 * 解析期貨歷史頁並強制驗證日期。
 */
function parseTaifexFutureHistoricalPage_(html, contractCode, dateText) {
  const plainText = normalizeWhitespace_(
    decodeHtmlEntities_(stripHtml_(html))
  );
  ...
  if (
    ...
    throw new Error('頁面不是 08:45–13:45 一般交易時段。');
  }
  ...
  if (pageDate !== dateText) {
    throw new Error(
      `期交所忽略歷史日期：要求 ${dateText}，頁面為 ${pageDate}`
    );
  }
  ...
  tableRows.forEach((cells) => {
    if (
      ...
      rows.push({
        ...
        contractMonth: String(cells[1] || '').trim().replace(/\s/g, ''),
        open: parseNumber_(cells[2]),
        high: parseNumber_(cells[3]),
        low: parseNumber_(cells[4]),
        close: parseNumber_(cells[5]),
        openInterest: parseNumber_(cells[12]),
        session: '一般交易時段',
      });
    }
  });
  ...
  if (!rows.length) {
    throw new Error(`${dateText} 找不到 ${contractCode} 日盤資料列。`);
  }

  return { date: pageDate, rows };
}

/**
 * 取得指定日期選擇權完整日盤 HTML。
 */
function fetchTaifexOptionsHistoricalHtml_(dateText) {
  const slashDate = isoToSlashDate_(dateText);
  const excelUrl =
    'https://www.taifex.com.tw/cht/3/optDailyMarketExcel';
  const reportUrl =
  ...
  const attempts = [
    {
      url:
        excelUrl +
        '?queryType=2&marketCode=0&commodity_id=TXO&queryDate=' +
        encodeURIComponent(slashDate),
      method: 'get',
    },
    {
      url:
        reportUrl +
        '?queryType=2&marketCode=0&commodityId=TXO&queryDate=' +
        encodeURIComponent(slashDate),
      method: 'get',
    },
    {
  ...
  for (const attempt of attempts) {
    try {
      ...
      if (pageDate !== dateText) {
        throw new Error(
          `期交所忽略選擇權歷史日期：要求 ${dateText}，頁面為 ${pageDate}`
        );
      }
  ...
  throw lastError || new Error(`${dateText} 選擇權歷史行情取得失敗。`);
}

/**
 * 解析指定日期三大法人 MTX / TMF 未平倉。
 */
function parseTaifexInstitutionalHtmlForDate_(html, dateText) {
  const plainText = normalizeWhitespace_(
    decodeHtmlEntities_(stripHtml_(html))
  );
  const dateMatch = plainText.match(
  ...
  if (pageDate !== dateText) {
    throw new Error(
      `三大法人頁面日期不符：要求 ${dateText}，取得 ${pageDate}`
    );
  }
  ...
  rows.forEach((cells) => {
    ...
    if (!Number.isFinite(longOi) || !Number.isFinite(shortOi)) return;
  ...
  ['MTX', 'TMF'].forEach((code) => {
    if (result[code].identities < 3) {
      throw new Error(`${code} ${dateText} 三大法人資料不完整。`);
    }
  });
...
/**
 * HTTP GET / POST 歷史查詢共用函式。
 */
function fetchHistoryHtmlAttempt_(attempt, encodings) {
  const options = {
    method: String(attempt.method || 'get').toLowerCase(),
    headers: {
      Accept: 'text/html,application/xhtml+xml,*/*',
      'User-Agent':
        'Mozilla/5.0 (compatible; GoogleAppsScript MarketHistoryRepair)',
    },
    followRedirects: true,
    muteHttpExceptions: true,
  };
  ...
  if (attempt.payload) {
    options.payload = attempt.payload;
  }

  const separator = attempt.url.includes('?') ? '&' : '?';
  const url =
    attempt.url +
    separator +
    '_ts=' +
  ...
  for (const encoding of encodings || ['UTF-8']) {
    try {
...
/**
 * ISO 日期轉期交所常用 YYYY/MM/DD。
 */
function isoToSlashDate_(dateText) {
  return String(dateText || '').replace(/-/g, '/');
}

function normalizeHistoryInputDate_(value) {
...
function daysBetweenIsoDates_(startText, endText) {
  const start = parseIsoDateAtNoon_(startText);
  const end = parseIsoDateAtNoon_(endText);
  if (!start || !end) return NaN;
  return Math.floor((end.getTime() - start.getTime()) / 86400000);
}

function saveHistoryRefreshState_(state) {
  PropertiesService.getDocumentProperties().setProperty(
    HISTORY_STATE_KEY,
...
function loadHistoryRefreshState_() {
...
function scheduleHistoricalRefreshContinuation_() {
  deleteTriggersByHandler_(HISTORY_TRIGGER_HANDLER);

  ScriptApp.newTrigger(HISTORY_TRIGGER_HANDLER)
    .timeBased()
    .after(Number(CONFIG.HISTORY_CONTINUE_DELAY_MS || 60000))
    .create();
}

function cancelHistoricalRefresh() {
  deleteTriggersByHandler_(HISTORY_TRIGGER_HANDLER);
  ...
  writeHistoryLog_('', '系統', '已取消', '使用者取消歷史修復。');
  SpreadsheetApp.getUi().alert('歷史修復已取消。');
}

function showHistoricalRefreshStatus() {
  const ui = SpreadsheetApp.getUi();
  const state = loadHistoryRefreshState_();
  ...
  ui.alert(
    ...
    [
      `狀態：${state.status || ''}`,
      `區間：${state.startDate || ''} ～ ${state.endDate || ''}`,
      `下一日期：${state.cursorDate || ''}`,
      `已處理交易日：${state.processedTradingDays || 0}`,
      `完整成功：${state.successfulDates || 0}`,
      `部分成功：${state.partialDates || 0}`,
      `完全失敗：${state.failedDates || 0}`,
      state.error ? `錯誤：${state.error}` : '',
    ].filter(Boolean).join('\n'),
    ui.ButtonSet.OK
  );
}

function writeHistoryLog_(dateText, source, result, message) {
  const ss = SpreadsheetApp.getActiveSpreadsheet();
  const sheet = getOrCreateSheet_(ss, CONFIG.HISTORY_LOG_SHEET);

  if (sheet.getLastRow() === 0) {
    initializeHeader_(sheet, HISTORY_LOG_HEADERS);
  }
