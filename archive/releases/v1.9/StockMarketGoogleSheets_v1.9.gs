...
const CONFIG = {
  ...
  // IV 為依官方最後成交價反推之模型估計值。為避免引入每日外部利率／股利資料，
  // 預設採 r=0、q=0；如有固定內部假設，可在此調整。
  IV_RISK_FREE_RATE: 0.0,
  IV_DIVIDEND_YIELD: 0.0,
  IV_ALERT_THRESHOLD: 35,
  IV_MIN_DAYS_TO_EXPIRY: 0.5,
...
const SUMMARY_GROUPS = [
  ...
  { start: 3, end: 7, label: '大盤現貨指數', fill: '#FFF2CC' },
  { start: 8, end: 13, label: '台指期貨指數', fill: '#FCE4D6' },
  { start: 14, end: 24, label: '週選擇權 W', fill: '#DDEBF7' },
  { start: 25, end: 35, label: '週選擇權 F', fill: '#D9EAD3' },
  { start: 36, end: 46, label: '月選擇權', fill: '#E4DFEC' },
  { start: 47, end: 49, label: '市場籌碼', fill: '#E2F0D9' },
  { start: 50, end: 56, label: '台指期未平倉（大額近月、法人所有月）', fill: '#F4B183' },
  { start: 57, end: 59, label: '更新資訊', fill: '#D9E1F2' },
];
...
function getOptionAtmContext_(date) {
  ...
  return {
    ...
    atmStrike: Math.round(atmReferencePrice / 100) * 100,
...
function attachOptionAtmMetrics_(summary, rows, atmContext, tradeDate) {
  ...
  summary.callLastPrice =
    callRow && Number.isFinite(callRow.lastPrice) && callRow.lastPrice > 0
      ? callRow.lastPrice
      : '';
  summary.putLastPrice =
    putRow && Number.isFinite(putRow.lastPrice) && putRow.lastPrice > 0
      ? putRow.lastPrice
      : '';

  const timeToExpiry = optionTimeToExpiryYears_(
    tradeDate,
    summary.expiryDate
  );

  summary.callIv = isFiniteNumber_(summary.callLastPrice)
    ? impliedVolatilityPercent_(
        'Call',
        atmContext.ivUnderlyingPrice,
        summary.atmStrike,
        Number(summary.callLastPrice),
        timeToExpiry
      )
    : '';
  summary.putIv = isFiniteNumber_(summary.putLastPrice)
    ? impliedVolatilityPercent_(
        'Put',
        atmContext.ivUnderlyingPrice,
        summary.atmStrike,
        Number(summary.putLastPrice),
        timeToExpiry
      )
    : '';
}
...
function optionTimeToExpiryYears_(tradeDate, expiryDate) {
  ...
  const calendarDays = (expiry.getTime() - trade.getTime()) / 86400000;
  const effectiveDays = Math.max(
    calendarDays,
    Number(CONFIG.IV_MIN_DAYS_TO_EXPIRY) || 0.5
  );
  return effectiveDays / 365;
}
...
function ensureSummarySheet_() {
  ...
  const versionReady =
    properties.getProperty('SUMMARY_V19_READY') === '1';
  const ready =
    sheet &&
    ...
    sheet.getRange('BE1').getDisplayValue() === '更新資訊';

  if (!ready || !versionReady) {
    const hasHistoricalRows = sheet && sheet.getLastRow() > 2;
    if (hasHistoricalRows) {
      // 僅在 v1.9 首次升級時重建一次，以正確插入新增欄位；日常排程仍採增量更新。
      syncSummaryTable_();
    } else {
      initializeSummarySheet_();
    }
    properties.setProperty('SUMMARY_V19_READY', '1');
  }
...
function initializeSummarySheet_() {
  ...
  SUMMARY_GROUPS.forEach((group) => {
    ...
    if (width === 1 && (group.start <= 2 || group.start >= 57)) {
  ...
  // 第二列子標題；垂直合併欄位不另外寫入，避免修改合併儲存格的一部分。
  SUMMARY_GROUPS.forEach((group) => {
    const width = group.end - group.start + 1;
    const isVerticalMerge = width === 1 && (group.start <= 2 || group.start >= 57);
  ...
  sheet
    ...
    .setVerticalAlignment('middle')
    .setWrap(true)
    .setBorder(true, true, true, true, true, true, '#808080', SpreadsheetApp.BorderStyle.SOLID);
  ...
  sheet.setColumnWidths(3, 11, 78);
  sheet.setColumnWidths(14, 33, 98);
  sheet.setColumnWidths(47, 10, 94);
  sheet.setColumnWidth(57, 142);
  sheet.setColumnWidth(58, 95);
  sheet.setColumnWidth(59, 330);
  ...
  sheet.getRange('I:M').setNumberFormat('#,##0.00');
  ...
  sheet.getRange('Z:Z').setNumberFormat('@');
  sheet.getRange('AA:AA').setNumberFormat('#,##0');
  sheet.getRange('AB:AB').setNumberFormat('@');
  sheet.getRange('AC:AC').setNumberFormat('#,##0');
  sheet.getRange('AD:AD').setNumberFormat('@');
  sheet.getRange('AE:AE').setNumberFormat('#,##0');
  sheet.getRange('AF:AF').setNumberFormat('#,##0.00');
  sheet.getRange('AG:AG').setNumberFormat('0.00');
  sheet.getRange('AH:AH').setNumberFormat('#,##0.00');
  sheet.getRange('AI:AI').setNumberFormat('0.00');

  // 月選 AJ:AT。
  sheet.getRange('AJ:AJ').setNumberFormat('@');
  sheet.getRange('AK:AK').setNumberFormat('@');
  sheet.getRange('AL:AL').setNumberFormat('#,##0');
  sheet.getRange('AM:AM').setNumberFormat('@');
  sheet.getRange('AN:AN').setNumberFormat('#,##0');
  sheet.getRange('AO:AO').setNumberFormat('@');
  sheet.getRange('AP:AP').setNumberFormat('#,##0');
  sheet.getRange('AQ:AQ').setNumberFormat('#,##0.00');
  sheet.getRange('AR:AR').setNumberFormat('0.00');
  sheet.getRange('AS:AS').setNumberFormat('#,##0.00');
  sheet.getRange('AT:AT').setNumberFormat('0.00');

  sheet.getRange('AU:AW').setNumberFormat('0.00');
  sheet.getRange('AX:BD').setNumberFormat('#,##0');
  sheet.getRange('BE:BE').setNumberFormat('yyyy/mm/dd hh:mm:ss');
  sheet.getRange('BF:BG').setNumberFormat('@');

  const positiveRanges = [
    sheet.getRange('G3:G'),
    sheet.getRange('M3:M'),
    sheet.getRange('AX3:BD'),
  ];
  ...
  const ivRanges = [
    sheet.getRange('V3:V'),
    sheet.getRange('X3:X'),
    sheet.getRange('AG3:AG'),
    sheet.getRange('AI3:AI'),
    sheet.getRange('AR3:AR'),
    sheet.getRange('AT3:AT'),
  ];
  const rules = [
    SpreadsheetApp.newConditionalFormatRule()
...
function buildSummaryRow_(record, previousSpotClose) {
  ...
  const spotChange =
    isFiniteNumber_(spot.close) && isFiniteNumber_(previousSpotClose)
      ? Number(spot.close) - Number(previousSpotClose)
      : '';
  const basis =
    isFiniteNumber_(future.close) && isFiniteNumber_(spot.close)
      ? Number(future.close) - Number(spot.close)
      : '';
  const complete =
    hasSummaryMarket_(spot) &&
    hasSummaryMarket_(future) &&
    hasSummaryChip_(chip) &&
    hasSummaryOptions_(option);
  const latestUpdate = record.updateTimes.length
    ? new Date(
        Math.max.apply(
          null,
          record.updateTimes.map((value) => value.getTime())
        )
      )
    : '';
...
function applySummaryRowStyles_(sheet, startRow, rowCount) {
  ...
  sheet
    .getRange(startRow, 59, rowCount, 1)
...
function syncSummaryTable_() {
  ...
  dates.forEach((date) => {
    ...
    const latestUpdate = record.updateTimes.length
      ? new Date(Math.max.apply(null, record.updateTimes.map((value) => value.getTime())))
      : '';
