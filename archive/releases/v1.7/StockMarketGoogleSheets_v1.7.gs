...
const SUMMARY_GROUPS = [
  ...
  { start: 14, end: 19, label: '週選擇權 W', fill: '#DDEBF7' },
  { start: 20, end: 25, label: '週選擇權 F', fill: '#D9EAD3' },
  { start: 26, end: 31, label: '月選擇權', fill: '#E4DFEC' },
  { start: 32, end: 34, label: '市場籌碼', fill: '#E2F0D9' },
  { start: 35, end: 41, label: '台指期未平倉（大額近月、法人所有月）', fill: '#F4B183' },
  { start: 42, end: 44, label: '更新資訊', fill: '#D9E1F2' },
];
...
function onOpen() {
  SpreadsheetApp.getUi()
    ...
    .addItem('重建總表', 'rebuildSummaryTable')
    .addToUi();
}
...
function initializeSheets() {
  ...
  consolidateDuplicateChipRows_(chipSheet);
  ...
  optionOiSheet.getRange('A:C').setNumberFormat('@');
  optionOiSheet.getRange('D:D').setNumberFormat('@');
  optionOiSheet.getRange('E:E').setNumberFormat('#,##0');
  optionOiSheet.getRange('F:F').setNumberFormat('@');
  optionOiSheet.getRange('G:G').setNumberFormat('#,##0');
  optionOiSheet.getRange('H:J').setNumberFormat('@');
  optionOiSheet.getRange('K:K').setNumberFormat('#,##0');
  optionOiSheet.getRange('L:L').setNumberFormat('@');
  optionOiSheet.getRange('M:M').setNumberFormat('#,##0');
  optionOiSheet.getRange('N:P').setNumberFormat('@');
  optionOiSheet.getRange('Q:Q').setNumberFormat('#,##0');
  optionOiSheet.getRange('R:R').setNumberFormat('@');
  optionOiSheet.getRange('S:S').setNumberFormat('#,##0');
  optionOiSheet.getRange('U:U').setNumberFormat('yyyy/mm/dd hh:mm:ss');
  consolidateDuplicateOptionOiRows_(optionOiSheet);
  optionOiSheet.autoResizeColumns(1, OPTION_OI_HEADERS.length);
...
function updateMarketData() {
  ...
  try {
    ...
    try {
      const summaryCount = syncSummaryTable_();
      results.push(`總表 ${summaryCount} 筆`);
    } catch (error) {
...
function fetchTaifexOptionOiFromWebsite_() {
  ...
  result.source = '臺灣期貨交易所官網－選擇權完整日盤行情';
  return result;
}
...
function fetchTaifexOptionOiFromOpenApi_() {
  ...
  const result = buildTaifexOptionOiSummary_(normalizedRows);
  result.source = '臺灣期貨交易所 OpenAPI－選擇權每日交易行情';
  return result;
}
...
function fetchTaifexOptionOiFromCsv_() {
  ...
  for (const attempt of attempts) {
    try {
      ...
      const result = buildTaifexOptionOiSummary_(normalizedRows);
      result.source = '臺灣期貨交易所 CSV－選擇權每日交易行情';
      return result;
    } catch (error) {
      lastError = error;
    }
...
function initializeSummarySheet_() {
  ...
  SUMMARY_GROUPS.forEach((group) => {
    ...
    if (width === 1 && (group.start <= 2 || group.start >= 42)) {
  ...
  SUMMARY_GROUPS.forEach((group) => {
    const width = group.end - group.start + 1;
    const isVerticalMerge = width === 1 && (group.start <= 2 || group.start >= 42);
  ...
  sheet.setColumnWidths(14, 18, 104);
  sheet.setColumnWidths(32, 10, 94);
  sheet.setColumnWidth(42, 142);
  sheet.setColumnWidth(43, 80);
  sheet.setColumnWidth(44, 330);
  ...
  sheet.getRange('AA:AA').setNumberFormat('@');
  sheet.getRange('AB:AB').setNumberFormat('#,##0');
  sheet.getRange('AC:AC').setNumberFormat('@');
  sheet.getRange('AD:AD').setNumberFormat('#,##0');
  sheet.getRange('AE:AE').setNumberFormat('@');
  sheet.getRange('AF:AH').setNumberFormat('0.00');
  sheet.getRange('AI:AO').setNumberFormat('#,##0');
  sheet.getRange('AP:AP').setNumberFormat('yyyy/mm/dd hh:mm:ss');
  sheet.getRange('AQ:AQ').setNumberFormat('@');
  sheet.getRange('AR:AR').setNumberFormat('@');

  const positiveRanges = [
    sheet.getRange('G3:G'),
    sheet.getRange('M3:M'),
    sheet.getRange('AG3:AO'),
  ];
...
function syncSummaryTable_() {
  ...
  if (output.length > 0) {
    ...
    summarySheet.getRange(3, 44, output.length, 1).setHorizontalAlignment('left').setWrap(true);
    summarySheet.getRange(3, 1, output.length, SUMMARY_SUB_HEADERS.length)
