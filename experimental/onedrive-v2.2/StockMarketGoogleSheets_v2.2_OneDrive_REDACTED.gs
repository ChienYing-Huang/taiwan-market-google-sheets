 ...
 * 4. 首次執行需授權
 *
 * v2.2：新增 OneDrive Excel 增量同步。Google「總表」仍為主資料源；
 *       授權 Microsoft Graph 後，只將本次新增／更新的日期列同步至
 *       OneDrive 測試活頁簿中的「總表同步測試」工作表。相同日期更新原列，
 *       新日期才插入新列，不每日重建整份 Excel。
 *
...
const CONFIG = {
  ...
  // ===== OneDrive Excel 同步（v2.2）=====
  // 測試階段先寫入新工作表，不修改 OneDrive 活頁簿內既有工作表。
  ONEDRIVE_SYNC_ENABLED: true,
  ONEDRIVE_TENANT: 'consumers',
  ONEDRIVE_SCOPE: 'offline_access Files.ReadWrite',
  ONEDRIVE_SHARE_URL:
    'REPLACE_WITH_ONEDRIVE_SHARE_URL',
  ONEDRIVE_WORKSHEET: '總表同步測試',
  ONEDRIVE_MAX_DATA_ROWS: 5000,
};
...
const SUMMARY_GROUPS = [
  ...
  { start: 19, end: 24, label: '週選擇權 W (OI)', fill: '#DDEBF7' },
  { start: 25, end: 30, label: '週選擇權 F (OI)', fill: '#D9EAD3' },
  { start: 31, end: 36, label: '月選擇權 (OI)', fill: '#E4DFEC' },
  { start: 37, end: 43, label: '週選擇權 W (IV)', fill: '#BDD7EE' },
  { start: 44, end: 50, label: '週選擇權 F (IV)', fill: '#C6E0B4' },
  { start: 51, end: 57, label: '月選擇權 (IV)', fill: '#D9E1F2' },
  { start: 58, end: 60, label: '市場籌碼', fill: '#E2F0D9' },
  { start: 61, end: 67, label: '台指期未平倉（大額近月、法人所有月）', fill: '#F4B183' },
];
...
function onOpen() {
  SpreadsheetApp.getUi()
    ...
    .addSeparator()
    .addItem('OneDrive：設定 Microsoft Client ID', 'configureOneDriveClientId')
    .addItem('OneDrive：開始授權', 'beginOneDriveAuthorization')
    .addItem('OneDrive：完成授權', 'completeOneDriveAuthorization')
    .addItem('OneDrive：測試連線', 'testOneDriveConnection')
    .addItem('OneDrive：同步最新總表列', 'testOneDriveSyncLatest')
    .addItem('OneDrive：清除授權', 'clearOneDriveAuthorization')
    .addToUi();
}
  ...
  try {
    ...
    if (summaryDates.size > 0) {
      try {
        ...
        // OneDrive 已完成授權時，僅同步本次實際新增／更新的日期。
        if (
          CONFIG.ONEDRIVE_SYNC_ENABLED &&
          isOneDriveAuthorized_() &&
          summaryResult.dates.length > 0
        ) {
          try {
            const oneDriveResult = syncSummaryDatesToOneDrive_(
              summaryResult.dates
            );
            results.push(
              `OneDrive新增 ${oneDriveResult.added} 列、更新 ${oneDriveResult.updated} 列`
            );
          } catch (oneDriveError) {
            errors.push(`OneDrive同步：${oneDriveError.message}`);
          }
        }
      } catch (error) {
...
function ensureSummarySheet_() {
  ...
  const versionReady =
    properties.getProperty('SUMMARY_V20_READY') === '1';
  const ready =
    sheet &&
    ...
    sheet.getRange('AK1').getDisplayValue() === '週選擇權 W (IV)' &&
    sheet.getRange('BI1').getDisplayValue() ===
      '台指期未平倉（大額近月、法人所有月）';

  if (!ready || !versionReady) {
    const hasHistoricalRows = sheet && sheet.getLastRow() > 2;
    if (hasHistoricalRows) {
      // v2.0 首次升級會重建一次總表以完成欄位遷移；日常仍採增量更新。
      syncSummaryTable_();
    } else {
      initializeSummarySheet_();
    }
    properties.setProperty('SUMMARY_V20_READY', '1');
  }
...
function initializeSummarySheet_() {
  ...
  sheet.getRange(1, 1, 2, columnCount).clearContent().clearFormat();
  ...
  sheet.setColumnWidths(19, 18, 98);
  sheet.setColumnWidths(37, 21, 94);
  sheet.setColumnWidths(58, 3, 96);
  sheet.setColumnWidths(61, 7, 94);
  ...
  [
    ...
    sheet.getRange(`${cols[5]}:${cols[5]}`).setNumberFormat('@');
  });

  // IV：W AK:AQ、F AR:AX、月 AY:BE。
  [
    ...
    sheet.getRange(`${cols[0]}:${cols[0]}`).setNumberFormat('@');
    sheet.getRange(`${cols[1]}:${cols[1]}`).setNumberFormat('#,##0');
    sheet.getRange(`${cols[2]}:${cols[2]}`).setNumberFormat('#,##0.00');
    sheet.getRange(`${cols[3]}:${cols[3]}`).setNumberFormat('0.00');
    sheet.getRange(`${cols[4]}:${cols[4]}`).setNumberFormat('#,##0.00');
    sheet.getRange(`${cols[5]}:${cols[5]}`).setNumberFormat('0.00');
    sheet.getRange(`${cols[6]}:${cols[6]}`).setNumberFormat('@');
  });

  sheet.getRange('BF:BH').setNumberFormat('0.00');
  sheet.getRange('BI:BO').setNumberFormat('#,##0');

  const directionalRanges = [
    sheet.getRange('G3:G'),
    sheet.getRange('L3:L'),
    sheet.getRange('R3:R'),
    sheet.getRange('BI3:BO'),
  ];
  const ivRanges = [
    sheet.getRange('AN3:AN'),
    sheet.getRange('AP3:AP'),
    sheet.getRange('AU3:AU'),
    sheet.getRange('AW3:AW'),
    sheet.getRange('BB3:BB'),
    sheet.getRange('BD3:BD'),
  ];
  const rules = [
    SpreadsheetApp.newConditionalFormatRule()
...
function buildSummaryRow_(record, previousSpotClose) {
  ...
  return [
    ...
    normalizeSheetDate_(weeklyW.expiry) || valueOrBlank_(weeklyW.expiry),
    ...
    normalizeSheetDate_(weeklyF.expiry) || valueOrBlank_(weeklyF.expiry),
    ...
    normalizeSheetDate_(monthly.expiry) || valueOrBlank_(monthly.expiry),
...
function syncSummaryTable_() {
  ...
  if (oldRowCount > 0) {
    summarySheet
      .getRange(3, 1, oldRowCount, SUMMARY_SUB_HEADERS.length)
      .clearContent();
  }
...
/* ========================================================================== */
/* OneDrive Excel 增量同步（v2.2）                                           */
/* ========================================================================== */

/**
 * 以 UserProperties 儲存 Microsoft OAuth 權杖。
 * 時間觸發器由建立者執行，因此可沿用同一使用者的 refresh token。
 */
const ONEDRIVE_PROP_KEYS = {
  CLIENT_ID: 'ONEDRIVE_CLIENT_ID',
  ACCESS_TOKEN: 'ONEDRIVE_ACCESS_TOKEN',
  REFRESH_TOKEN: 'ONEDRIVE_REFRESH_TOKEN',
  ACCESS_TOKEN_EXPIRES_AT: 'ONEDRIVE_ACCESS_TOKEN_EXPIRES_AT',
  DEVICE_CODE: 'ONEDRIVE_DEVICE_CODE',
  DEVICE_CODE_EXPIRES_AT: 'ONEDRIVE_DEVICE_CODE_EXPIRES_AT',
  DEVICE_CODE_INTERVAL: 'ONEDRIVE_DEVICE_CODE_INTERVAL',
  SHARE_URL_OVERRIDE: 'ONEDRIVE_SHARE_URL_OVERRIDE',
  ITEM_ID: 'ONEDRIVE_ITEM_ID',
  DRIVE_ID: 'ONEDRIVE_DRIVE_ID',
  ITEM_NAME: 'ONEDRIVE_ITEM_NAME',
  WORKSHEET_ID: 'ONEDRIVE_WORKSHEET_ID',
};

/**
 * 從試算表選單輸入 Microsoft Entra 應用程式的 Application (client) ID。
 * 不需要 client secret；本工具使用 Device Code Flow。
 */
function configureOneDriveClientId() {
  const ui = SpreadsheetApp.getUi();
  const response = ui.prompt(
    '設定 Microsoft Client ID',
    '請貼上 Microsoft Entra App registration 的 Application (client) ID。',
  ...
  const clientId = String(response.getResponseText() || '').trim();

  if (!/^[0-9a-fA-F-]{30,40}$/.test(clientId)) {
    ui.alert('Client ID 格式看起來不正確，請確認後再試一次。');
    return;
  }

  PropertiesService.getUserProperties().setProperty(
    ONEDRIVE_PROP_KEYS.CLIENT_ID,
    clientId
  );
  ui.alert('Microsoft Client ID 已儲存。接著執行「OneDrive：開始授權」。');
}

/**
 * 開始 Microsoft Device Code Flow。
 * 使用者需到 Microsoft 顯示的驗證網址輸入一次性代碼。
 */
function beginOneDriveAuthorization() {
  const ui = SpreadsheetApp.getUi();
  const clientId = getOneDriveClientId_();

  const response = UrlFetchApp.fetch(
    `https://login.microsoftonline.com/${CONFIG.ONEDRIVE_TENANT}/oauth2/v2.0/devicecode`,
    {
      method: 'post',
      payload: {
        client_id: clientId,
        scope: CONFIG.ONEDRIVE_SCOPE,
      },
      muteHttpExceptions: true,
    }
  );
  ...
  if (code < 200 || code >= 300) {
    throw new Error(
      `Microsoft 授權初始化失敗 HTTP ${code}：${extractGraphErrorMessage_(body)}`
    );
  }

  const props = PropertiesService.getUserProperties();
  props.setProperty(ONEDRIVE_PROP_KEYS.DEVICE_CODE, body.device_code);
  props.setProperty(
    ONEDRIVE_PROP_KEYS.DEVICE_CODE_EXPIRES_AT,
    String(Date.now() + Number(body.expires_in || 900) * 1000)
  );
  props.setProperty(
    ONEDRIVE_PROP_KEYS.DEVICE_CODE_INTERVAL,
    String(body.interval || 5)
  );
  ...
  ui.alert(
    'Microsoft OneDrive 授權',
    [
      `1. 開啟：${verificationUri}`,
      `2. 輸入代碼：${body.user_code}`,
      '3. 使用擁有／可編輯測試 Excel 的 Microsoft 帳號登入並同意權限。',
      '4. 完成後回到試算表，執行「OneDrive：完成授權」。',
      '',
      `代碼有效時間約 ${Math.round(Number(body.expires_in || 900) / 60)} 分鐘。`,
    ].join('\n'),
    ui.ButtonSet.OK
  );
}
...
/**
 * 完成 Device Code Flow，取得 access token 與 refresh token。
 */
function completeOneDriveAuthorization() {
  const ui = SpreadsheetApp.getUi();
  const props = PropertiesService.getUserProperties();
  const clientId = getOneDriveClientId_();
  const deviceCode = props.getProperty(ONEDRIVE_PROP_KEYS.DEVICE_CODE);

  if (!deviceCode) {
    ui.alert('找不到待完成的授權。請先執行「OneDrive：開始授權」。');
    return;
  }

  const expiresAt = Number(
    props.getProperty(ONEDRIVE_PROP_KEYS.DEVICE_CODE_EXPIRES_AT) || 0
  );

  if (expiresAt && Date.now() >= expiresAt) {
    clearOneDriveDeviceCode_();
    ui.alert('一次性授權代碼已過期，請重新執行「OneDrive：開始授權」。');
    return;
  }

  const response = UrlFetchApp.fetch(
    `https://login.microsoftonline.com/${CONFIG.ONEDRIVE_TENANT}/oauth2/v2.0/token`,
    {
      method: 'post',
      payload: {
  ...
  if (code >= 200 && code < 300 && body.access_token) {
    storeOneDriveTokens_(body);
    clearOneDriveDeviceCode_();

    const item = resolveOneDriveWorkbook_(true);
    const worksheet = ensureOneDriveWorksheet_(item);
    ensureOneDriveHeaders_(item, worksheet);

    ui.alert(
      'OneDrive 授權完成',
      [
        `Excel：${item.name || '(未取得檔名)'}`,
        `同步工作表：${worksheet.name}`,
        '',
        '接著可執行「OneDrive：同步最新總表列」進行第一次測試。',
      ].join('\n'),
      ui.ButtonSet.OK
    );
    return;
  ...
  if (errorCode === 'authorization_pending') {
    ui.alert(
      'Microsoft 尚未收到完成授權的結果。請先在瀏覽器完成登入與同意，再重新執行「OneDrive：完成授權」。'
    );
    return;
  }

  if (errorCode === 'slow_down') {
    ui.alert('Microsoft 要求稍後再查詢。完成登入後再重新執行「OneDrive：完成授權」。');
    return;
  }

  throw new Error(
    `Microsoft 授權失敗 HTTP ${code}：${extractGraphErrorMessage_(body)}`
  );
}

/**
 * 測試 Graph 是否能解析指定 OneDrive Excel，並確認目標工作表。
 */
  ...
  try {
    ...
    ui.alert(
      ...
      [
        ...
        `工作表：${worksheet.name || ''}`,
        '',
        '尚未寫入行情資料。可再執行「OneDrive：同步最新總表列」。',
      ].join('\n'),
      ui.ButtonSet.OK
    );
  } catch (error) {
    ui.alert(`OneDrive 連線失敗：\n${error.message}`);
    throw error;
  }
}

/**
 * 手動測試：同步 Google「總表」中最新一個交易日。
 */
function testOneDriveSyncLatest() {
  const ui = SpreadsheetApp.getUi();
  const sheet = SpreadsheetApp.getActiveSpreadsheet().getSheetByName(
    CONFIG.SUMMARY_SHEET
  );
  ...
  if (!sheet || sheet.getLastRow() <= 2) {
    ui.alert('Google「總表」目前沒有可同步的資料列。');
    return;
  }
  ...
  if (!values.length) {
    ui.alert('Google「總表」找不到有效資料日期。');
    return;
  }

  const date = values[values.length - 1];
  const result = syncSummaryDatesToOneDrive_([date]);
...
/**
 * 清除本 Apps Script 所保存的 Microsoft OAuth 權杖與 OneDrive 快取資訊。
 * 不會刪除 OneDrive 檔案，也不會撤銷 Microsoft 帳戶中的應用授權。
 */
function clearOneDriveAuthorization() {
  const ui = SpreadsheetApp.getUi();
  const props = PropertiesService.getUserProperties();

  Object.keys(ONEDRIVE_PROP_KEYS).forEach((key) => {
    if (key === 'CLIENT_ID') return;
    props.deleteProperty(ONEDRIVE_PROP_KEYS[key]);
  });

  ui.alert('已清除 Apps Script 中的 OneDrive 授權權杖與檔案快取。Client ID 仍保留。');
}

/**
 * OneDrive 是否已具備可續期的 OAuth 授權。
 */
function isOneDriveAuthorized_() {
  const props = PropertiesService.getUserProperties();
  return Boolean(
    props.getProperty(ONEDRIVE_PROP_KEYS.REFRESH_TOKEN) ||
    props.getProperty(ONEDRIVE_PROP_KEYS.ACCESS_TOKEN)
  );
}

/**
 * 將指定日期的 Google「總表」資料增量同步至 OneDrive Excel。
 * 新日期依時間順序插入；相同日期更新原列。
 */
function syncSummaryDatesToOneDrive_(rawDates) {
  if (!CONFIG.ONEDRIVE_SYNC_ENABLED) {
    return { added: 0, updated: 0, dates: [] };
  }

  if (!isOneDriveAuthorized_()) {
    throw new Error(
      '尚未完成 Microsoft 授權。請先從「台股行情工具」執行 OneDrive 授權。'
    );
  }
  ...
  const item = resolveOneDriveWorkbook_(false);
  const worksheet = ensureOneDriveWorksheet_(item);
  ensureOneDriveHeaders_(item, worksheet);
  ...
  dates.forEach((date) => {
    const googleRow = getGoogleSummaryRowForOneDrive_(date);

    if (!googleRow) {
      return;
    }

    const rowInfo = findOneDriveDateRow_(item, worksheet, date);
    let targetRow = rowInfo.row;

    if (rowInfo.exists) {
      updated += 1;
    } else {
      if (rowInfo.insertBeforeExisting) {
        insertOneDriveSummaryRow_(item, worksheet, targetRow);
      }
      added += 1;
    }

    writeOneDriveSummaryRow_(
      item,
      worksheet,
      targetRow,
      sanitizeOneDriveValues_(googleRow)
    );
  });
...
/**
 * 取得 Google 總表指定日期的 67 欄資料。
 */
function getGoogleSummaryRowForOneDrive_(date) {
  const sheet = SpreadsheetApp.getActiveSpreadsheet().getSheetByName(
    CONFIG.SUMMARY_SHEET
  );
  ...
  const row = findSummaryRowByDate_(sheet, date);
  if (row <= 0) return null;

  return sheet
    .getRange(row, 1, 1, SUMMARY_SUB_HEADERS.length)
    .getValues()[0];
}

/**
 * 解析使用者提供的 OneDrive 分享／文件連結，取得 DriveItem。
 */
function resolveOneDriveWorkbook_(forceRefresh) {
  const props = PropertiesService.getUserProperties();

  if (!forceRefresh) {
    const cachedId = props.getProperty(ONEDRIVE_PROP_KEYS.ITEM_ID);
    const cachedDriveId = props.getProperty(ONEDRIVE_PROP_KEYS.DRIVE_ID);
    const cachedName = props.getProperty(ONEDRIVE_PROP_KEYS.ITEM_NAME);
  ...
  const shareUrl =
    props.getProperty(ONEDRIVE_PROP_KEYS.SHARE_URL_OVERRIDE) ||
    CONFIG.ONEDRIVE_SHARE_URL;
  const sharingToken = encodeOneDriveSharingUrl_(shareUrl);

  const response = oneDriveGraphFetch_(
    `/shares/${encodeURIComponent(sharingToken)}/driveItem`,
    'get'
  );

  const item = response.body || {};
  const extension = String(item.name || '').toLowerCase();

  if (!item.id) {
    throw new Error('Microsoft Graph 未回傳 OneDrive Excel 的 Item ID。');
  }

  if (extension && !extension.endsWith('.xlsx')) {
    throw new Error(
      `目前檔案為「${item.name}」。Excel Graph 同步需要 .xlsx 活頁簿。`
    );
  }

  const driveId =
    item.parentReference && item.parentReference.driveId
      ? item.parentReference.driveId
      : '';

  props.setProperty(ONEDRIVE_PROP_KEYS.ITEM_ID, item.id);
  props.setProperty(ONEDRIVE_PROP_KEYS.DRIVE_ID, driveId);
  props.setProperty(ONEDRIVE_PROP_KEYS.ITEM_NAME, item.name || '');
 ...
 * 確保測試工作表存在；不存在就建立。
 */
function ensureOneDriveWorksheet_(item) {
  const props = PropertiesService.getUserProperties();
  const cachedWorksheetId = props.getProperty(
    ONEDRIVE_PROP_KEYS.WORKSHEET_ID
  );

  if (cachedWorksheetId) {
    return {
      id: cachedWorksheetId,
      name: CONFIG.ONEDRIVE_WORKSHEET,
    };
  }

  const listResponse = oneDriveWorkbookFetch_(
    item,
    '/worksheets',
    'get'
  );
  const worksheets = Array.isArray(listResponse.body.value)
    ? listResponse.body.value
    : [];
  let worksheet = worksheets.find(
    (sheet) => String(sheet.name || '') === CONFIG.ONEDRIVE_WORKSHEET
  );

  if (!worksheet) {
    const createResponse = oneDriveWorkbookFetch_(
      item,
      '/worksheets/add',
      'post',
      { name: CONFIG.ONEDRIVE_WORKSHEET }
    );
    worksheet = createResponse.body;
  }

  if (!worksheet || !worksheet.id) {
    throw new Error(
      `無法取得或建立 OneDrive 工作表「${CONFIG.ONEDRIVE_WORKSHEET}」。`
    );
  }
  ...
  return {
    id: worksheet.id,
    name: worksheet.name || CONFIG.ONEDRIVE_WORKSHEET,
  };
}

/**
 * 初始化 OneDrive 測試工作表的雙層表頭。
 * 為避免測試階段破壞既有版面，不做儲存格合併，只寫入相同欄位順序。
 */
function ensureOneDriveHeaders_(item, worksheet) {
  const lastColumn = columnNumberToLetters_(SUMMARY_SUB_HEADERS.length);
  const headerAddress = `A1:${lastColumn}2`;
  const current = readOneDriveRange_(item, worksheet, headerAddress);
  const currentValues = Array.isArray(current.values) ? current.values : [];

  const row1 = new Array(SUMMARY_SUB_HEADERS.length).fill('');
  SUMMARY_GROUPS.forEach((group) => {
    row1[group.start - 1] = group.label;
  });

  const row2 = SUMMARY_SUB_HEADERS.slice();
  row2[0] = '';
  row2[1] = '';

  const needsWrite =
    !currentValues.length ||
    String(currentValues[0] && currentValues[0][0] || '') !== '資料日期' ||
    String(currentValues[0] && currentValues[0][2] || '') !== '大盤現貨指數' ||
    String(currentValues[1] && currentValues[1][2] || '') !== '開盤';
...
/**
 * 尋找 OneDrive 測試表中的指定日期。
 * 若日期不存在，回傳依日期升冪應插入的位置。
 */
function findOneDriveDateRow_(item, worksheet, date) {
  const endRow = Math.max(3, Number(CONFIG.ONEDRIVE_MAX_DATA_ROWS || 5000));
  const range = readOneDriveRange_(
    item,
    worksheet,
    `A3:A${endRow}`
  );
  ...
  let lastNonBlankRow = 2;

  for (let index = 0; index < values.length; index += 1) {
    const rowNumber = index + 3;
    const existingDate = normalizeOneDriveDate_(values[index] && values[index][0]);
...
/**
 * 在 A:BO 範圍插入一列並將下方資料往下推，以保持日期升冪。
 */
function insertOneDriveSummaryRow_(item, worksheet, rowNumber) {
  const lastColumn = columnNumberToLetters_(SUMMARY_SUB_HEADERS.length);
  const address = `A${rowNumber}:${lastColumn}${rowNumber}`;

  oneDriveWorkbookFetch_(
    item,
    `/worksheets/${encodeURIComponent(worksheet.id)}/range(address='${address}')/insert`,
    'post',
    { shift: 'Down' }
  );
}
 ...
 * 寫入單一總表資料列。
 */
function writeOneDriveSummaryRow_(item, worksheet, rowNumber, rowValues) {
  const lastColumn = columnNumberToLetters_(SUMMARY_SUB_HEADERS.length);
  const address = `A${rowNumber}:${lastColumn}${rowNumber}`;
  writeOneDriveRange_(item, worksheet, address, [rowValues]);
}

/**
 * 讀取 OneDrive Excel range。
 */
function readOneDriveRange_(item, worksheet, address) {
  const response = oneDriveWorkbookFetch_(
    item,
    `/worksheets/${encodeURIComponent(worksheet.id)}/range(address='${address}')`,
    'get'
  );
...
/**
 * 更新 OneDrive Excel range values。
 */
function writeOneDriveRange_(item, worksheet, address, values) {
  return oneDriveWorkbookFetch_(
    item,
    `/worksheets/${encodeURIComponent(worksheet.id)}/range(address='${address}')`,
    'patch',
    { values }
...
/**
 * 呼叫 Excel workbook API。
 * 優先使用 driveId + itemId，若無 driveId 則使用 /me/drive/items/{id}。
 */
function oneDriveWorkbookFetch_(item, workbookPath, method, payload) {
  let basePath;

  if (item.driveId) {
    basePath =
      `/drives/${encodeURIComponent(item.driveId)}` +
      `/items/${encodeURIComponent(item.id)}/workbook`;
  } else {
    basePath =
      `/me/drive/items/${encodeURIComponent(item.id)}/workbook`;
  }

  return oneDriveGraphFetch_(
    `${basePath}${workbookPath}`,
    method,
    payload
  );
}
...
/**
 * Microsoft Graph 通用請求。
 */
function oneDriveGraphFetch_(path, method, payload) {
  const token = getOneDriveAccessToken_();
  ...
  if (code < 200 || code >= 300) {
    throw new Error(
      `Microsoft Graph HTTP ${code}：${extractGraphErrorMessage_(body)}`
    );
  }

  return { code, body, headers: response.getAllHeaders() };
}

/**
 * 取得有效 access token；過期時以 refresh token 自動續期。
 */
function getOneDriveAccessToken_() {
  const props = PropertiesService.getUserProperties();
  const accessToken = props.getProperty(ONEDRIVE_PROP_KEYS.ACCESS_TOKEN);
  const expiresAt = Number(
    props.getProperty(ONEDRIVE_PROP_KEYS.ACCESS_TOKEN_EXPIRES_AT) || 0
  );
  ...
  const refreshToken = props.getProperty(ONEDRIVE_PROP_KEYS.REFRESH_TOKEN);

  if (!refreshToken) {
    throw new Error('OneDrive OAuth 權杖不存在，請重新完成 Microsoft 授權。');
  }
  ...
  const response = UrlFetchApp.fetch(
    `https://login.microsoftonline.com/${CONFIG.ONEDRIVE_TENANT}/oauth2/v2.0/token`,
    {
      method: 'post',
      payload: {
        client_id: getOneDriveClientId_(),
  ...
  if (code < 200 || code >= 300 || !body.access_token) {
    throw new Error(
      `OneDrive 權杖續期失敗 HTTP ${code}：${extractGraphErrorMessage_(body)}`
    );
  }
...
/**
 * 儲存 Microsoft access/refresh token。
 */
function storeOneDriveTokens_(body) {
  const props = PropertiesService.getUserProperties();

  if (body.access_token) {
    props.setProperty(
      ONEDRIVE_PROP_KEYS.ACCESS_TOKEN,
      body.access_token
    );
    props.setProperty(
      ONEDRIVE_PROP_KEYS.ACCESS_TOKEN_EXPIRES_AT,
      String(Date.now() + Math.max(Number(body.expires_in || 3600) - 120, 60) * 1000)
    );
  }

  if (body.refresh_token) {
    props.setProperty(
      ONEDRIVE_PROP_KEYS.REFRESH_TOKEN,
      body.refresh_token
    );
  }
}

function getOneDriveClientId_() {
  ...
  if (!clientId) {
    throw new Error(
      '尚未設定 Microsoft Client ID。請先執行「OneDrive：設定 Microsoft Client ID」。'
    );
  }
...
function clearOneDriveDeviceCode_() {
  const props = PropertiesService.getUserProperties();
  props.deleteProperty(ONEDRIVE_PROP_KEYS.DEVICE_CODE);
  props.deleteProperty(ONEDRIVE_PROP_KEYS.DEVICE_CODE_EXPIRES_AT);
  props.deleteProperty(ONEDRIVE_PROP_KEYS.DEVICE_CODE_INTERVAL);
}

/**
 * Graph Shares API 要求：u! + unpadded base64url(sharing URL)。
 */
function encodeOneDriveSharingUrl_(url) {
...
/**
 * Google Sheets / Graph 之間的資料型態正規化。
 */
function sanitizeOneDriveValues_(row) {
...
function normalizeOneDriveDate_(value) {
  ...
  // Excel REST 有時可能回傳 Excel serial date。
...
function columnNumberToLetters_(columnNumber) {
...
function extractGraphErrorMessage_(body) {
  if (!body) return '未知錯誤';
